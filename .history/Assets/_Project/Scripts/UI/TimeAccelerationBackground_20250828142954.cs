...
public class TimeAccelerationBackground : MonoBehaviour
{
    [Header("滤镜材质")]
    ...
    private void InitializeSystem()
    {
        ...
        {
            Debug.LogError("[TimeAccelerationBackground] 找不到Image组件！");
            return;
        }
        ...
        {
            Debug.LogError("[TimeAccelerationBackground] 时间加速滤镜材质未设置！请拖入TimeAccelerationUI.mat");
            return;
        }

        // 创建材质实例
        instanceMaterial = new Material(timeAccelerationMaterial);
        instanceMaterial.name = "TimeAccelerationUI (Instance)";
        ...
        if (showDebugLogs)
        {
            Debug.Log("[TimeAccelerationBackground] 背景滤镜系统初始化完成");
        }
    }
    ...
    private void OnMadeInHeavenStateChanged(bool isActive)
    {
        if (showDebugLogs)
        {
            Debug.Log($"[TimeAccelerationBackground] 天堂制造状态变化: {(isActive ? "激活" : "停用")}");
        }
    ...
    public void StartTimeAccelerationFilter()
    {
        ...
        if (showDebugLogs)
        {
            Debug.Log("[TimeAccelerationBackground] 背景时间加速滤镜已启动");
        }
    }
    ...
    public void StopTimeAccelerationFilter()
    {
        ...
        if (showDebugLogs)
        {
            Debug.Log("[TimeAccelerationBackground] 背景时间加速滤镜已停止");
        }
    }
    ...
    private void UpdateMaterialProperties()
    {
        ...
        {
            Debug.Log($"[TimeAccelerationBackground] 材质参数已更新 - 强度: {currentIntensity:F2}");
        }
    }
    ...
    public void SetFilterIntensity(float intensity)
    {
        ...
        // 根据强度决定使用哪个材质
        backgroundImage.material = isFilterActive ? instanceMaterial : originalMaterial;

        UpdateMaterialProperties();

        if (showDebugLogs)
        {
            Debug.Log($"[TimeAccelerationBackground] 滤镜强度设置为: {intensity:F2}");
        }
    }
    ...
    private void HandleDebugInput()
    {
        ...
        {
            Debug.Log("[TimeAccelerationBackground] F4键 - 强制设置高强度滤镜");
    ...
    {
        Debug.Log("[TimeAccelerationBackground] 测试最大强度滤镜");
        SetFilterIntensity(1.0f);
    }
    ...
    {
        return $"背景时间加速滤镜: {(isFilterActive ? "激活" : "停用")} (强度: {currentIntensity:F2})";
    }
